<table id="ba_dashboard_table">
    <tbody>
        ...
        <tr><th><?php _e('visitors with AdBlocker'); ?></th><td class="<?php if($benchmark_general && ($blocked > $benchmark_general || ($blocked > $benchmark_category))) echo 'warning'; else echo 'success'; ?>"><?php echo $blocked; ?>%</td></tr>
        <tr><th><?php _e('general benchmark'); ?></th><td><?php echo $benchmark_general; ?>%</td></tr>
        <tr><th><?php _e('category benchmark'); ?></th><td><?php echo $benchmark_category; ?>%</td></tr>
        <?php if(!in_array( 'basic', $this->_active_stat_methods )) : ?><tr><th><?php _e('measuring'); ?></th><td><?php _e('deactivated', BATD); ?></td></tr><?php endif; ?>
    </tbody>
</table>
<?php if ( $this->_compare_allowed ) : ?><p class="success"><?php _e('You can now compare your data with others'); ?></p><?php endif; ?>
<p>
