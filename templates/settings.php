...
<div class="abc-form-block">
    <form action="" method="post">
        ...
        <span class="start"><?php _e('started at:', ABCOUNTERTD); ?><br/><?php echo date_i18n(get_option('date_format'), get_option('abc_start')) . ', ' . date_i18n('H:i', get_option('abc_start'));?></span>
        <?php else : ?>
...
<div class="abc-form-block">
    <form action="" method="post">
        ...
        if ( !empty( $stop ) ) : ?>
        <span class="stop"><?php _e('stopped at:', ABCOUNTERTD); ?><br/><?php echo date_i18n(get_option('date_format'), get_option('abc_stop')) . ', ' . date_i18n('H:i', get_option('abc_stop'));?></span>
        <?php else : ?>
